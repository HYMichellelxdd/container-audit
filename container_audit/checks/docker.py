...
class DockerChecks:
    ...
    def check_docker_socket_permissions(self, info: dict) -> Finding:
        """Check if Docker socket has correct permissions."""
        mounts = info.get("Mounts", [])
        for m in mounts:
            dst = m.get("Destination", "")
            if "/var/run/docker.sock" in dst:
                mode = m.get("Mode", "")
                is_readonly = "ro" in mode.lower() if mode else False
                return Finding(
                    check_id="DOCKER-015",
                    title="Docker socket permissions",
                    severity=Severity.HIGH,
                    status=Status.PASS if is_readonly else Status.WARN,
                    description=(
                        f"Docker socket mounted read-only."
                        if is_readonly else
                        "Docker socket mounted read-write (consider read-only)."
                    ),
                    remediation="Mount Docker socket as read-only: /var/run/docker.sock:/var/run/docker.sock:ro",
                    evidence=f"Mode: {mode or 'rw (default)'}",
                )
        return Finding(
            check_id="DOCKER-015",
            title="Docker socket permissions",
            ...
            status=Status.PASS,
            description="Docker socket not mounted.",
        )
    ...
    def check_compose_service(self, name: str, config: dict[str, Any]) -> list[Finding]:
        ...
        # Healthcheck
        findings.append(self.check_compose_healthcheck(name, config))
        # Restart policy
        findings.append(self.check_compose_restart(name, config))
        # Read-only filesystem
        findings.append(self.check_compose_read_only(name, config))

        return findings


    def check_compose_healthcheck(self, svc_name: str, config: dict) -> Finding:
        """Check if compose service has healthcheck."""
        has_healthcheck = "healthcheck" in config
        return Finding(
            check_id=f"COMPOSE-{svc_name}-HEALTH",
            title=f"Service '{svc_name}': healthcheck",
            severity=Severity.LOW,
            status=Status.PASS if has_healthcheck else Status.WARN,
            description=(
                f"Service '{svc_name}' has healthcheck configured."
                if has_healthcheck else
                f"Service '{svc_name}' has no healthcheck."
            ),
            remediation="Add healthcheck to improve reliability.",
        )

    def check_compose_restart(self, svc_name: str, config: dict) -> Finding:
        """Check restart policy."""
        restart = config.get("restart", "no")
        is_unsafe = restart in ("always", "unless-stopped")
        return Finding(
            check_id=f"COMPOSE-{svc_name}-RESTART",
            title=f"Service '{svc_name}': restart policy '{restart}'",
            severity=Severity.LOW,
            status=Status.WARN if is_unsafe else Status.PASS,
            description=f"Service '{svc_name}' restart policy: {restart}",
            remediation="Use 'on-failure:3' instead of 'always' for production.",
        )

    def check_compose_read_only(self, svc_name: str, config: dict) -> Finding:
        """Check if root filesystem is read-only."""
        read_only = config.get("read_only", False)
        tmpfs = config.get("tmpfs", [])
        return Finding(
            check_id=f"COMPOSE-{svc_name}-ROFS",
            title=f"Service '{svc_name}': read-only filesystem",
            severity=Severity.LOW,
            status=Status.PASS if read_only else Status.WARN,
            description=(
                f"Service '{svc_name}' has read-only filesystem."
                if read_only else
                f"Service '{svc_name}' has writable root filesystem."
            ),
            remediation="Add 'read_only: true' and 'tmpfs: [/tmp, /var/run]' for security.",
        )
