...
class KubernetesChecks:
    ...
    def check_security_context(self, container: dict, kind: str) -> list[Finding]:
        """Check container security context best practices."""
        findings = []
        ...
        if "allowPrivilegeEscalation" not in sc:
            findings.append(Finding(
                check_id=f"K8S-PE-{name}",
                title=f"Container '{name}': privilege escalation not explicitly disabled",
                severity=Severity.MEDIUM,
                status=Status.WARN,
                description=f"allowPrivilegeEscalation not set for '{name}'.",
        ...
        if "seccompProfile" not in sc:
            findings.append(Finding(
                check_id=f"K8S-SECCOMP-{name}",
                title=f"Container '{name}': no seccomp profile",
                severity=Severity.LOW,
                status=Status.WARN,
                description=f"No seccomp profile set for '{name}'.",
                remediation="Set seccompProfile.type: RuntimeDefault or RuntimeDefault.",
            ))
        ...
        if "ALL" not in dropped:
            findings.append(Finding(
                check_id=f"K8S-CAPDROP-{name}",
                title=f"Container '{name}': capabilities not dropped",
                severity=Severity.MEDIUM,
                status=Status.WARN,
                description=f"Capabilities not fully dropped for '{name}'.",
